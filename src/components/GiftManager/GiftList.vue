  ...
  <div class="account-transactions">
    ...
    <div class="accountInfo" v-if="mainActive === true">
      <table class="table-setting">
        <tr>
          <th class="txt-left">Gift Address</th>
          <th class="txt-left">Recipient</th>
          <th class="txt-left">Message Data</th>
          <!-- <th class="txt-left">Transaction</th> -->
          <th class="txt-left">Deadline</th>
          <!-- <th class="txt-left">Amount</th> -->
          <th class="txt-left">Info</th>
        </tr>
        <tr v-for="(item, index) in transactions" :key="index" v-show="item.gift === true">
          <td class="txt-left">{{ $utils.maskAddress(item.signer.address.pretty()) }}</td>
          <td class="txt-left">{{ (item.recipient === account.address.address) ? account.name : $utils.maskAddress(item.recipient.pretty()) }}</td>
          <td class="txt-left">{{ item.msgData }}</td>
          <!-- <td class="txt-left">{{ `${$utils.getNameTypeTransaction(item.type)} Gift` }}</td> -->
    ...
    async getAccounts (id = null) {
      ...
      if (myAccounts !== null) {
        ...
        try {
          ...
          const dataStructure = await this.$utils.getStructureGift(transactions, this.$config, this.$provider)
